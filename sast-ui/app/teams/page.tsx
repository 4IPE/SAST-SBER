            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {teams.map((team) => (
                    <Card key={team.id}
                        ...
                        <CardContent>
                            <p className="text-sm text-text-secondary mb-1">Количество
                                участников: {team.amountTeammates}</p>
                            ...
                            <div className="flex flex-wrap gap-2">
                                <Link href={`/teams/${team.id}`} className="flex-1 min-w-0">
                                    <Button variant="outline" className="w-full group truncate">
                                        Посмотреть команду
                                        <ChevronRight
                                            className="ml-2 h-4 w-4 transition-transform group-hover:translate-x-1"/>
                                    </Button>
                                </Link>
                                <Link href={`/projects/${team.project.id}`} className="flex-1 min-w-0">
                                    <Button variant="outline" className="w-full group truncate">
                                        Посмотреть проект
